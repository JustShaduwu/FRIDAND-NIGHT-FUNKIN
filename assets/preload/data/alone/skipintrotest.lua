...
function onCreatePost()
	makeLuaText('skip', 'Presiona ESPACIO para saltar la intro', 1270, 0, 200);
	addLuaText('skip');
	setTextSize('skip', 30)
...
function onUpdatePost()
    if getPropertyFromClass('flixel.FlxG', 'keys.justPressed.SPACE') and not getProperty('startingSong') and didskip == false then
            didskip = true
			removeLuaText('skip');
            setPropertyFromClass('Conductor', 'songPosition', 18500) -- it is counted by milliseconds, 1000 = 1 second
