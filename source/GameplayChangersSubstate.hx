...
class GameplayChangersSubstate extends MusicBeatSubstate
{
	...
	function getOptions()
	{
		var goption:GameplayOption = new GameplayOption(Language.scrolltype, 'scrolltype', 'string', 'multiplicative', ["multiplicative", "constant"]);
		optionsArray.push(goption);

		var option:GameplayOption = new GameplayOption(Language.scrollspeed, 'scrollspeed', 'float', 1);
		...
		var option:GameplayOption = new GameplayOption(Language.healthloss, 'healthloss', 'float', 1);
		option.scrollSpeed = 2.5;
		option.minValue = 0.5;
		option.maxValue = 5;
		option.changeValue = 0.1;
		...
		var option:GameplayOption = new GameplayOption(Language.instakill, 'instakill', 'bool', false);
		optionsArray.push(option);

		var option:GameplayOption = new GameplayOption(Language.practice, 'practice', 'bool', false);
		optionsArray.push(option);
	...
	override function update(elapsed:Float)
	{
		...
		if(nextAccept <= 0)
		{
			...
			if(usesCheckbox)
			{
				if(controls.ACCEPT)
				{
					if (curOption.getValue() == true)
						FlxG.sound.play(Paths.themeSound('optionOff'));
					else if (curOption.getValue() == false)
						FlxG.sound.play(Paths.themeSound('optionOn'));
