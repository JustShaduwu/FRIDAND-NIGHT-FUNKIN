...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Set GF Speed', "Sets GF head bopping speed,\nValue 1: 1 = Normal speed,\n2 = 1/2 speed, 4 = 1/4 speed etc.\nValue 2: Character Speed to change (Dad, BF, GF)\nUsed on Fresh during the beatbox parts.\n\nWarning: Value must be integer!"],
		...
		['Subtitles', "Value 1:\n   0: Desactivate\n   Any other number: The line to read\n\nValue 2: The color in HEX (if null, white)"],
		['Set Property', "Value 1: Variable name\nValue 2: New value"],
		['Change Mania', "Work in progress\nValue 1: The new mania value (min: 0; max: 9)"],
		['Change Icon', "Vaule 1: Character, dad or bf\nValue 2: Icon"],
	];
	...
	**/
	...
	var wavefordropdown:FlxUIDropDownMenuCustom;
	var songsparts:Array<String> = ['Main/Player', 'Inst', 'Opponent'];
	#end
	...
	function addChartingUI() {
		...
		if (FlxG.save.data.chart_waveformTrack == null) FlxG.save.data.chart_waveformTrack = 'Inst';
		...
		wavefordropdown = new FlxUIDropDownMenuCustom(waveformEnabled.x + 120, waveformEnabled.y,
			FlxUIDropDownMenuCustom.makeStrIdLabelArray(songsparts, true), function(track:String)
		{
			FlxG.save.data.chart_waveformTrack = songsparts[Std.parseInt(track)];
			updateWaveform();
		});
		wavefordropdown.selectedLabel = FlxG.save.data.chart_waveformTrack;
		blockPressWhileScrolling.push(wavefordropdown);
		#end
		...
		check_warnings = new FlxUICheckBox(check_vortex.x + 120, check_vortex.y, null, null, "Ignore Progress Warnings", 100);
		...
		var check_mute_opp_vocals = new FlxUICheckBox(check_mute_inst.x + 120, check_mute_inst.y + 30, null, null, "Mute Opponent Vocals (in editor)", 100);
		...
		check_mute_opp_vocals.callback = function()
		{
			if (oppVocals != null)
			{
				...
				if (check_mute_opp_vocals.checked)
					vol = 0;

				oppVocals.volume = vol;
			}
		};
		...
		tab_group_chart.add(wavefordropdown);
		#end
	...
	function loadSong():Void
	{
		...
		var file:Dynamic = Paths.voices(currentSongName, _song.player1);
		var oppFile:Dynamic = Paths.oppVoices(currentSongName);
		vocals = new FlxSound();
		...
		{
			oppVocals.loadEmbedded(oppFile);
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			if (FlxG.keys.justPressed.ENTER)
			{
				...
				if (oppVocals != null) oppVocals.stop();
			...
			if (FlxG.keys.justPressed.SPACE)
			{
				if (FlxG.sound.music.playing)
				{
					...
					if (oppVocals != null) oppVocals.pause();
				}
				else
				{
					if(vocals != null) {
					...
					if (oppVocals != null)
					{
						oppVocals.play();
						oppVocals.pause();
						oppVocals.time = FlxG.sound.music.time;
						oppVocals.play();
					}
			...
			if(vortex && !blockInput){
				var controlArray:Array<Array<Bool>> = [
					...
					[
						...
						FlxG.keys.justPressed.FIVE,
						FlxG.keys.justPressed.SIX, FlxG.keys.justPressed.SEVEN, FlxG.keys.justPressed.EIGHT, FlxG.keys.justPressed.NINE,
					...
					if (curSelectedNote != null)
					{
						var controlArray:Array<Array<Bool>> = [
							...
							[
								...
								FlxG.keys.justPressed.NINE, FlxG.keys.justPressed.ZERO
							],
							[false],
							[false],
							[false],
	...
	*/
	...
	function updateWaveform() {
		...
		if (FlxG.save.data.chart_waveformTrack == 'Inst') {
		...
		if (FlxG.save.data.chart_waveformTrack == 'Main/Player') {
		...
		if (FlxG.save.data.chart_waveformTrack == 'Opponent') {
			var sound:FlxSound = oppVocals;
			if (sound._sound != null && sound._sound.__buffer != null) {
				var bytes:Bytes = sound._sound.__buffer.data.toBytes();
	...
	function loadJson(song:String):Void
	{
		...
		var folder:String = song.toLowerCase();
		...
		if (CoolUtil.difficulties[PlayState.storyDifficulty] != CoolUtil.defaultDifficulty) {
			if(CoolUtil.difficulties[PlayState.storyDifficulty] == null){
				jsonInput = song.toLowerCase();
			}else{
				jsonInput = song.toLowerCase() + "-" + CoolUtil.difficulties[PlayState.storyDifficulty];
			}
		}
		else{
			jsonInput = song.toLowerCase();
		...
		if (!Song.checkJson(jsonInput, folder)) {
			openSubState(new Prompt('We couldn\'t find the file, check again.', 0, function() {
				//resetDiffLabel();
			}, null, ignoreWarnings, "OK", ""));
		}
		else {
