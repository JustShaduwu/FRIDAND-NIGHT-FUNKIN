...
*/
class PortalTunesState extends MusicBeatState
{
	...
	override function create()
	{
		...
		#if MODS_ALLOWED
		path = Paths.modFolders("images/tunes");
		if (FileSystem.exists(path)) {
			for (file in FileSystem.readDirectory(path)) {
				if (file.endsWith(".json")) {
					albums.push(Json.parse(Paths.getTextFromFile("images/tunes" + '/' + file)));
				}
			}
		}
        #end
		...
		albumCover = new FlxSprite().loadGraphic(Paths.image('tunes/$diskImg'));
   ...
   FlxG.switchState(new PortalTunesLALState());
	...
	//		FlxG.unfocuPause = ClientPrefs.unfocuPause;
		...
		if (supaDirection == 'right') {
			FlxTween.tween(albumCover, {x: FlxG.width}, 0.15, {
				...
				onComplete: function(twn:FlxTween)
				{
					...
					albumCover.loadGraphic(Paths.image('tunes/$diskImg'));
