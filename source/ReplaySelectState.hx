...
class ReplaySelectState extends MusicBeatState
{
    ...
    override function create()
    {
        ...
        var path:Array<String> = [Paths.getPreloadPath('replays/')];
        ...
        if (path != null && path.length > 0)
        {
            for (i in 0...path.length)
            {
                ...
                #else
                var replayFile:ReplayFile = Json.parse(OpenFlAssets.getText(Paths.getPreloadPath('replays/$file')));
                #end
    ...
    override function update(elapsed:Float)
    {
        ...
        else if (controls.ACCEPT && menuItems.length > 0)
        {
            ...
            LoadingState.loadAndSwitchState(new ReplayState(menuItems[curSelected].split(" ")[1], difficulties[curSelected]), true);
        }
